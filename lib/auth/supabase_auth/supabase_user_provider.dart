...
class FashionHubSupabaseUser extends BaseAuthUser {
  FashionHubSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> fashionHubSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FashionHubSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
